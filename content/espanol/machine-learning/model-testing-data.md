...
Ahora estamos usando el modelo entrenado para estimar jugadores en `test_data`. De forma similar a lo que hicimos con `train_data`, creamos `x_test` y `y_test`.

`model.predict()` generará una lista de resultados predichos.
...
Comparemos con las valoraciones reales "Overall"

```python
# añadir una nueva columna de "Predicted Overall" a test_data
test_data['Predicted Overall'] = y_pred.copy()

# añadir una nueva columna de la diferencia porcentual de la predicción a test_data
...
¿No es asombroso? Con este resultado, estarás confiado para usar este modelo y estimar la valoración general (Overall) de cualquier jugador de fútbol del mundo.

Ahora vamos a graficarlo para visualizarlo.
...
# añadir ticks, etiquetas y leyenda
plt.xticks(())
